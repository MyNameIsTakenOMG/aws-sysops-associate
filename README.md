...
- #### Lecture:
  - ec2:
    ...
    - enhanced networking: high bandwidth, high packet per second
      - sr-iov
      - elastic fabric adapter: inter-node communication, only for linux
    - shutdown behavior: how should the instance react when shutdown is done using the os? stop(default), terminate. not applicable when shutting down from aws console
    - termination protection: aws console, cli
    - launch troubleshooting: `instanceLimitExceeded` (vCPUs for each type), `insufficientinstanceCapacity`(aws out of capacity), `instance terminates immediately` (ebs, ami, check ec2 console)
    - ec2 spot instance requests:
      - max spot price
      - spot block
      - to stop spot instances: cancel spot request first, then the instances
    - spot fleets: set of spot instances + optional on-demand instances
      - define launch pools
      - strategies to allocate spot instances:
        - prices, diversitied, capacity, price&capacity
    - burstable instance(t2.t3): cpu credits
    - t2,t3 unlimited
    - cloudwatch metrics for ec2: custom metric resolution up to 1 sece
    - cloudwatch agent: collect additional system-level metrics, such as ram, and logs
      - procstat plugin
    - ec2 hibernate: root ebs must be encrypted, not instance store, 60 days, must be less than 150 gb
  - AMI
    - region-scoped
    - no-reboot option (by default, disabled for data integrity)
      - aws backup does not reboot(not help guarantees data integrity)
    - cross-account ami copy: owner must grant read permissions for the ebs snapshot
    - combine with aws config to find non-compliant ec2 instnace
  - management for ec2 (SSM)
    - manage ec2 and on-prem
    - works for linux and windows
    - aws tags
    - resource groups (based on tags)
    - ssm -- document
    - ssm -- run command: a document, or a command
    - ssm -- automation: automation runbook, ssm documents of type automation
    - ssm -- parameter store: standard and advanced(parameters policies)
    - ssm -- inventory: collect metadata from your managed instances(ec2 or on-prem)
    - ssm -- state manager: keep your managed instances in a state
      - association: define a state that ec2 or on-prem should stay
      - use ssm documents to create an association
    - ssm -- patch manager: patch on-demand or scheduled using `maintenance windows`
      - patch baseline: pre-defined, custom
      - patch group: tag key: `Patch Group`, one patch baseline, one patch group, one instance
  ...
  - HA & scalability
    - load balancer
      ...
      - types:
        ...
        - alb:
          - target groups: ec2, ecs, lambda, ip addresses(must be private)
          - fixed hostname
          - to client ip, check header: `x-forwarded-for`
        - gateway lb:
          - a fleet of 3rd-party network(firewall, intrusion detection, packet inspection,...)
          - target groups: ec2, private ip
      - sticky session: not for gateway load balancer
        - cookies: application-based cookies, duration-based cookies
      - cross-zone load balancing:
        - alb: default
        - nlb, gateway lb: disabled, no free
        - clb: disabled
      - ssl certificates:
        - using ACM or upload your own certificates
        - server name indication (SNI, for client to specify)
        - clb(one ssl cert), alb & nlb (multiple certs)
      - connection draining: de-registering while waiting for in-flight requests
      - health check: if there is only unhealthy instances, then send requests anyway
      - monitoring: `HTTPCode_ELB_5XX`, `HTTPCode_ELB_4XX` generated by load balancer4
      - for request tracing: it only adds a header: `X-Amzn-Trace-Id`, but not integrated with x-ray
      - slow start mode: time to warm-up for new instance
      - request routing algorithms:
        - least outstanding requests
        - round robin
        - flow hash (nlb)
      - target group weighting: select a weight for each group on a single rule
    - asg:
      - a launch template
      - scaling policies:
        - dynamic scaling: target tracking, simple/step tracking
        - scheduled scaling
        - predictive scaling
      - scaling cooldown: could use golden ami
      - life hooks
      - health check: asg will not reboot unhealthy but terminate it and then launch a new one
      - cloudwatch metrics for asg:
        - collected every 1 min
        - asg level (opt-in)
        - ec2 level enabled
      - aws auto scaling: asg, spot fleet requests, ecs, dynamodb, aurora
        - dynamic scaling
        - predictive scaling
  - Elastic beanstalk
  ...
  - cloudformation
    - building blocks:
      - resources(mandatory)
      - outputs: `Export` section
      - condition: cannot be applied to `parameter` section
    - intrinsic functions:
      - fn::ref --> reference a parameter
    - rollbacks
    - service role (iam::PassRole permission)
    - capabilities: iam resources, auto expend (for Macros or nested stacks), `insufficientCapabilitiesException`(when capabilities have not been acknowledged when deploying a template)
    - deletionPolicy: to protect resources
    - stack policy: define update actions allowed during stack updates
    - termination protection: to protect the whole stack
    - custom resources
    - dynamic references: SSM parameter store & Secret manager
      - `ManageMasterUserPassword`: create admin secret implicitly for RDS, aurora
      - `dynamic reference`
    - helper scripts: cfn-init, cfn-signal, cfn-hup,...
      - wait conditions: attach a `creationPolicy`
      - not receive enough signals: download scripts, check logs while disable rollback on failure, make sure the instance internet connection...
    - dependson: applied when using fn::ref and fn::getatt
    - stacksets
    - stackset permission models:
      - self-managed
      - service-managed
    - stackset with aws organization
      - auto-deploy stack to new aws account in the organization
    - cloudformation troubleshooting:
      - delete_failed
      - update rollback failed
    - stackset troubleshooting
      - insufficient permissions in target accounts
      - trying to create a global resources that must be unique, like s3
      - no trust relationship
      - reaches the limit in target account
  - lambda
    - pricing: per call, per duration and GB
    - s3 event notifications
    - lambda execution role: managed, custom
    - resource-based policy
    - logging & monitoring
    - tracing with x-ray
    - configuration:
      - increase RAM, vCPU will be increased accordingly
      - 1,792 MB a function will have a full vCPU, after it, considering using multi-threading in the code
    - execution context: temporary runtime environment to initialize any dependencies, sdk clients, http cliens,... it includes `/tmp` directory. remember to initialization outside the handler
    - /tmp space: 10GB
    - concurrency and throttling: can set `reserved concurrency`
    - concurrency and asynchronous invocations(retry)
    - cold start & provisioned concurrency
    - cloudwatch metrics, alarms, logs, and log insights(query all lambda logs)
    - lambda insights: collect system-level metrics, diagnostic info
  - ec2 storage & data management
    - ebs volume: network drive, az scoped, have a provisioned capacity. 30 GB free tier
    - ec2 instance store: high-performance but will be deleted when the instance stopped
    - ebs types:
      - gp2/gp3 (root volume)
      - io1/io2 (root volume): support ebs multi-attach, 16 ec2 instances at a time in the same az. each instance has full read&write
      - st1: frequent access
      - sc1: infrequent access
    - ebs resizing: can only increase size, after it, you need to repartition your drive. cannot decrease it(create a new one and migrate data)
    - ebs snapshot: no need to detach volume, but recommended
    - aws data lifecycle manager: manage ebs snapshots and ebs-backed ami; cannot manage snapshots or amis outside DLM, cannot manage instance-store backed ami
    - ebs snapshots -- fast snapshot restore
      - using `dd` or `fio` command to pre-warm the volume
      - or using FSR, but expensive
    - ebs snapshot features:
      - archive
      - recycle bin
    - ebs migration
    - ebs encryption: using kms (AES 256)
    - EFS:
      - multi-az 100s of instances, linux based ami, using kms
      - performance:
        - performance mode: general purpose, max i/o
        - throughput mode: bursting, provisioned, elastic
      - storage classes: standard, IA, archive, lifecycle policies
      - access points: Restrict access to a directory within the file system and optionally specify a different root directory
      - operations: lifecycle policy, performance mode, access points, data migration using `dataSync`
      - cloudwatch metrics: `percentIOLimit`, `BurstCreditBalance`,`storageBytes`
  - s3
    - objects(files) storage
    - global unique name
    - regional level service
    - objects:
      - size: 5TB
      - must use `multi-part upload` if greater than 5GB
    - security: user-based iam policies, bucket policies, object access control list
    - bucket policies
    - static website hosting, make sure the bucket name is the same as the custom domain name
    - versioning: bucket level
    - replication (same-region or cross-region)
      - only new objects will be replicated after enabling replication
      - or using s3 `batch replication`
      - can replicate delete markers
      - there is no chainning of replication
    - storage classes
      - intelligent-tiering
  - s3 advanced
    - lifecycle rules to automatically move objects between storage classes
      - transition actions
      - expiration actions
    - storage class analysis: recommandations for standard and IA. good first step to put together lifecycle rules
    - baseline performance:
      - 3,500 put/copy/post/delete or 5,500 get/head request per second per prefix
    - performance:
      - multi-part upload
      - s3 transfer acceleration
      - s3 byte-range fetches
    - batch operations: s3 inventory to get object list and s3 select to filter your objects
    - s3 inventory: list objects with their metadata
    - s3 glacier: each item called `archive`, `archives` are stored in `vaults` and encrypted at rest using AES-256
    - s3 glacier operations:
      - vault operations
      - glacier operations
      - retrieval options
    ...
    - aws athena
      ...
      - performance improvement
        ...
        - partition
        - use larger files
      - federated query (data source connector -- lambda)
  - s3 security
    - object encryption:
      - sse-s3: "x-amz-server-side-encryption": "AES256"
      - sse-kms: audit, auto-rotation, "x-amz-server-side-encryption": "aws:kms"
      - sse-c: https required with encryption key
      - cse: Use client libraries such as Amazon S3 Client-Side Encryption Library
    - encryption in transit: ssl/tls
    - bucket policies evaluated before default encryption
    - s3 -- cors: for cross-region request
    - MFA delete:
      - suspend versioning, permanently delete objects
      - enable versioning, list deleted versions
    - access log
    - pre-signed urls
    - vault lock:
      - retention mode: compliance / gevernance
      - legal hold
      - adopt a WORM model
      - retention period: can be extended
    - access points:
      - own DNS name with an access policy
      - vpc origin: vpc endpoint - access point - s3 bucket
        - vpc endpoint policy must allow access point and s3 bucket
    - multi-region access points:
      - failover control
  - advanced storage solutions
    - snow family device (dataSync used under the hood)
    - edge computing: preprocess data, ML, transcoding media
    - aws FSx: launch 3rd party high-performance file systems on aws
      - for windows: support SMB and NTFS, can be mounted on linux ec2 instances
      - for lustre: high performance computing (linux cluster), can read s3 as a file system through FSx
        - scratch file system
        - persistent file system (data replicated in the same az)
      - for netapp ontap: support NFS, SMB, iSCSI. move ontap to aws
      - for openzfs: support NFS, move zfs to aws
    - FSx for windows:
      - single-az
      - multi-az: synchronous replication, auto-failover
    - aws storage gateway
      - s3 file gateway: SMB, NFS, most recently used data -- cached
      - fsx file gateway: local cache for frequently accessed data, windows compatible
      - volume gateway: iSCSI protocol
        - cached volumes: quickly access most recent data
        - stored volumes: whole dataset on-prem, scheduled backups to s3
      - tape gateway: iSCSI interface, virtual tape library backed by s3 and glacier
    - gateway hardware appliance: aws provisioned on-prem virtualization
    - storage gateway -- SYSOPS:
      - file gateway is linux file system compliant
    ...
    - storage gateway -- volume gateway cache
      ...
      - cache efficiency: `CacheHitPercent`(want it to be high), `CachePercentUsed`(do not want it to be high)
      - create a larger cache disk: clone the cached volume to a larger size, and then select the new disk as the cached volume
  - cloudfront
    - CDN, DDoS protection, integrated with aws shield, aws waf
    - origins:
    ...
    - access logs
    - troubleshooting:
      - 4xx: object not found, unauthorized
      - 5xx: gateway issues
    - caching:
      - headers
      - session cookies
    ...
    - caching TTL:
      - `cache-control:max-age` from origins
    - separate static and dynamic distributions
    - increase cache hit rate: `cache-control: max-age`, minimally required headers, cookies, query string parameters, separate static and dynamic distributions
    - with ALB sticky sessions: must forward or whitelist the cookies used for session affinity; set ttl less than when the cookie expires 
  - databases
    - RDS: managed sql database service, but cannot ssh to the underlying db instances
      - storage auto scaling: setup maximum storage threshold, used for unpredictable workloads
      - read replicas: up to 15 read replicas within az, cross az, cross regions, replication is async, can be promoted to their own DB, applications must update the connection strings
      - network cost: cross-region not free
      - multi-az(DR): sync, failover standby(one DNS name). NOTE: read replicas can be setup as multi-az for DR
      - from single-az to multi-az: snapshot, restore, replication sync
      - failover conditions:
        - primary db down
        - az outage
        - manual failover using Reboot with failover
      - lambda: launched outside vpc, cannot access private resources, thus deploy it in the vpc, then an ENI will be created
      - RDS proxy for aws lambda: pool the database connections
      - DB parameter groups:
        - dynamic parameter applied immediately
        - static parameter applied after reboot
        - must reboot after modifying parameter groups
        - force SSL connections: `PostgreSQL / SQL Server: rds.force_ssl=1`, `MySQL / MariaDB: require_secure_transport=1`
      - rds backup vs snapshots
      - rds snapshots sharing:
        - manual snapshots: can be shared
        - automated snapshots: cannot be shared, copy first
        - share unencrypted or encrypted with cmk(must share cmk first)
      - rds events & event subscriptions
        - events about db instances, snapshots, sg, parameter groups,...
        - subscription: sns
        - rds deliver events to eventbridge
        - rds deliver logs to cloudwatch logs
      - RDS with cloudwatch
...
- #### Tests:
  ...
  - for storage gateway, create a new gateway with a new cache disk, do not remove the original cache disk
  - aws config rules can be triggered by configuration changes or periodic(schedule)
  - load balancer port: 1024 -- 65535
  - ebs volume: st1 for intensive throughput
  - s3 glacier: 10GB free retrieval
  - elb access logs to check client info: ip, request path,response, latency
  - stacksets: self-managed(trust policy), service-managed
  - aurora cluster to be public: configure db subnet group for public subnets (public ips), enable vpc dns resolution, dns hostnames
  - cloudtrail log file integrity validation -- digest file
  - aws service catalog -- account-to-account share; redeploy to another account
  - ec2 instance gets deleted by `terminateInstances` api will not publish `deleteVolume` api, which is used by aws config
  - asg launch ec2 using encrypted ami, be sure the cmk is shared
  - aws opsHub for snow family devices
  - aws rds proxy pool the connections
  - once enable multi-az for load balancer, you cannot disable it
  - when restoring ebs snapshot, using pre-warm to get rid of initial latency or using fast snapshot recovery which is expensive
  - s3 bucket owner has no permission on object created by other account, object owner must grant permission first
  - dynamodb integrated with sql databases using dynamodb stream and lamdba
  - aws cloudHSM: hardware security module is an aws provided self-managed key module
  - efs: `clientConnections` metric
  - fsx for windows: support SMB, NTFS, AD
  - asg scaling policy:
    - on demand
    - on schedule
    - on prediction
  - aws shield advanced: 24/7 support
  - s3 static website: make sure the bucket name is the same as record name
  - AMI two virutalization types:
    - paravirtual: not support for all aws region
    - hardware virtual machine    
  - aws-issued cert cannot be installed on ec2, only 3rd-party cert
  - make sure cert is in the same aws region as acm requests
  - when enable sticky session, make sure have cloudfront to forward necessary cookies
  - Amazon S3 Access Points is a feature of S3 that simplifies managing data access at scale for applications using shared data sets on S3.  each access point contains a unique DNS name
  - Amazon EFS access points are application-specific entry points into an EFS file system that makes it easier to manage application access to shared datasets.
  - CloudFront always caches a few of the HTTP 4xx and 5xx status codes returned by your origin. CloudFront doesn't generate 404 responses.
  - aws secrets manager: RDS, redshift, documentDB
  - for ELB: httpcode_4xx_count: generated by ELB not the target
  - aws firewall manager can manage multiple aws accounts and applications in your aws organizations
  - cloudtrail digest logs: encrypted using sse-s3
  - for acm, we  cannot download pirvate key
  - Amazon RDS provides metrics in real time for the operating system (OS) that your DB instance runs on.
  - ebs volume: raid 0( high i/o), raid 1 (fault tolerant)
  - create AMIs without `no reboot`
  - aws backup will do with `no reboot`
  - s3 bucket owner needs to give the root user permission to access to the bucket. But, the root user can change the bucket policy
  - s3 pre-signed url
  - cognito identity pool -- sts token
  - asg -- instance protection
  - ec2 -- termination protection (not prevent from shutdown within)
  - spot instance (not applicable for termination protection)
  - aws cloudformation -- cfn-init, cfn-signal, wait condition
  - cloudwatch agent configuration files(should be different names, or overwrite)
  - in EB, asg uses two default alarms to trigger scaling actions, but can be configured
  - for vpc flow logs, once created, it cannot be changed, must create a new one
  - aws storage gateway: encryption at rest, in-transit
  - cloudformation stack: `set onFailure=do_nothing`
  - s3 object retention:
    - can be set on each version of every object
    - use s3 bucket default setting
    - use `Retain until data` for object version
  - rds paramter group: can configure connection encryption   
  - stack policy is just for stack updates, not for access
  - s3 bucket access operation + cognito federated user (sts)
  - common tag --> cost allocation tags
  - ALB is not integrated with x-ray, only add a header to every request`X-Amzn-Trace-Id`
  - when ebs stay in `attaching` status, try to re-attach it with a different device name
  - stack -- termination protection
  - s3 metric `5xx` server error
  - for RDS and aurora, to enable logs:
    - RDS, enable `slow query log`, `general log`, `audit log`
    - aurora, modify the cluster parameter groups 
  - s3 replication: S3 RTC by default includes S3 replication metrics and S3 event notifications,
  - cloudfront key pair can only be created by the root user
  - ec2 service can interrupt spot instances:
    - stop
    - hibernate
    - terminate
    - **NOTE**: no reboot
  - s3: multi-part upload, s3 transfer accelerator
  - aws kms: cmk deletion pending period
  - sqs metric dimension: sqs queue name
  - s3 access problem: run SSM automation document: `AWSSupport-TroubleshootS3PublicRead`
  - wanna test out new DB engine version, then create a snapshot, and create a new db instance out of the snapshot, and then test the new version
  - sse-kms: provide audit trail, sse-s3 does not
  - secrets manager: to bypass the deletion pending period, use api call `ForceDeleteWithoutRecovery`
  - sse-s3: AES-256
  - to check the client ip from ELB forwarded requests, check the header:`X-Forwarded-For`
